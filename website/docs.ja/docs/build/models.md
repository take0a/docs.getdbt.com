...
SQL は、ほとんどの dbt ユーザーが使用する言語ですが、モデル構築に使用できる唯一の言語ではありません。バージョン 1.3 以降、dbt Core と <Constant name="cloud" /> は Python モデルをサポートしています。Python モデルは、データ サイエンス モデルのトレーニングやデプロイ、複雑な変換、または特定の Python パッケージがニーズを満たす場合に役立ちます。たとえば、日付を解析するために `dateutil` ライブラリを使用するなどです。
