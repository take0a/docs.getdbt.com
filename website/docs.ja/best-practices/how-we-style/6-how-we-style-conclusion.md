...
dbt プロジェクトのスタイル設定方法を確認したところで、次は独自のスタイルを構築してみましょう。このガイドをコピーして、自分のプロジェクトのテンプレートとして自由に使用できます。そうした場合、ぜひご意見をお聞かせください。スタイル ガイドを共有するには、[コミュニティ フォーラム](https://discourse.getdbt.com/c/show-and-tell/22) または [Slack](https://www.getdbt.com/community) でご連絡ください。貢献者が簡単にフォローできるように、スタイル ガイドをコードと同じ場所に配置することをお勧めします。GitHub を使用している場合は、リポジトリの wiki にスタイル ガイドを追加するか、README に含めることができます。

## コミット前のフック

最後に、チームに追加の精神的負担をかけずに、スタイル ガイドの自動化されたルールが確実に守られるようにするには、[コミット前フック](https://pre-commit.com/) を使用して、コミット前にコードにスタイル違反がないか自動的にチェックし (多くの場合、自動的に修正します) ます。これは、すべての貢献者がスタイル ガイドに従っていることを確認するための優れた方法です。スタイル ガイドを決定して公開し、コードベースがそれに準拠したら、これを実装することをお勧めします。これにより、今後のすべてのコミットがスタイル ガイドに従うようになります。コミュニティから、dbt 用の優れたオープン ソース コミット前フックのセットを見つけることができます [dbt-checkpoint プロジェクト](https://github.com/dbt-checkpoint/dbt-checkpoint)。
