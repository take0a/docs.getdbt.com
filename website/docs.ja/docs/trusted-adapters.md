...
信頼できるアダプタは、信頼できるアダプタ プログラムに参加しており、プログラムの要件を満たす義務を負っています。信頼できるアダプタは、dbt Labs、パートナー、コミュニティ メンバーによって管理されています。<Constant name="cloud" />  の信頼できるアダプタは、開発、ドキュメント、ユーザー エクスペリエンス、メンテナンスの要件を網羅する厳格な追加プロセスを経ています。実稼働環境で使用することをお勧めします。詳細については、[信頼できるとは](/guides/adapter-creation?step=8#what-it-means-to-be-trusted) を参照してください。
